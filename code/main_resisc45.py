...
def test(epoch, num_class, current_weight, TensorSensing, VectorSensing):
    ...
    if acc > best_acc:
        ...
        if TensorSensing == False and VectorSensing  == False:
            torch.save(model.state_dict(), './checkpoint/pretrain_resisc45.pth')
        torch.save(state, './checkpoint/MCL_resisc45.pickle')
        current_weight = True
        best_acc = acc
...
sample_file_path = os.path.join(image_folder_path, 'pretrain_resisc45.pth')
if not os.path.exists(image_folder_path) or not os.path.exists(sample_file_path):
    ...
    for lr, epoch in zip(LR, Epoch):
        ...
        if current_weight:
            model.load_state_dict(torch.load('./checkpoint/pretrain_resisc45.pth'))
...
if args.mode == "tensor":
    for inputs, labels in train_loader:
        ...
        W1, W2, W3 = HOSVD(inputs, 235, 235, 1, device)#(334,334,1), (235, 235, 1), (163, 163, 1)
        break
    TensorSensing = TensorSensing((235, 235, 1), W1, W2, W3, W1.T, W2.T, W3.T, linear_sensing=False, separate_decoder=True, regularizer=None, constraint=None, device=device)
    VectorSensing = False
    ...
    for inputs, labels in train_loader:
        ...
        sample_B = sample[:,2,:,:]
        height = 8
        R_p = PCA(sample_R, height, device = device)
        G_p = PCA(sample_G, height, device = device)
        B_p = PCA(sample_B, height, device = device)
